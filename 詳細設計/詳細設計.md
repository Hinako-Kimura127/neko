...
@Service public class ProductService {
    ...
    public List<ProductListItem> findAllProducts() {
        // 商品一覧を取得し、ProductListItem DTOへ変換して返却    }

    public ProductDetail findProductById(Integer productId) {
        // 商品IDで商品を取得し、ProductDetail DTOへ変換して返却    }
}
...
@Service public class OrderService {
    ...
    public OrderResponse placeOrder(OrderRequest orderRequest, HttpSession session) {
        // カート情報とOrderRequest（顧客情報）から注文情報を生成しDBへ保存// 商品在庫の更新を行う// カートをクリア// 注文IDと日時をOrderResponseで返却    }
}

#### 4.4.4. ProductRepository
```java
@Repository public interface ProductRepository extends JpaRepository<Product, Integer> {
    // findAll(): 商品一覧取得// findById(Integer productId): 商品詳細取得}
```

#### 4.4.5. OrderRepository
```java
@Repository public interface OrderRepository extends JpaRepository<Order, Integer> {
    // save(Order): 注文情報登録}
```

#### 4.4.6. OrderDetailRepository
```java
@Repository public interface OrderDetailRepository extends JpaRepository<OrderDetail, Integer> {
    // saveAll(List<OrderDetail>): 注文明細一括登録}
```
